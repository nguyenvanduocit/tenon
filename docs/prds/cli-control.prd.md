...
| 2026-08-14 | The v3 control socket does not add a capability-advertisement handshake. Exact protocol-version rejection is sufficient for this closed wire: the five control actions are fixed, domain operations are versioned intent names discovered with their schemas, and any protocol-major change must prove rejection in both skew directions. | A numeric version without behavior is weak, but this socket never guesses across versions: the decoder rejects before action interpretation, and the two-way codec fixture covers an older client against the current host plus a newer client against an older host. A future open-ended control surface would require named advertisement instead. | adding a sixth control action or a generic “current” version alias |
