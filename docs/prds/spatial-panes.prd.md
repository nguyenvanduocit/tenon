...
| pane-owner public adapter | [`CoreIntentName.swift`](../../Sources/TenonCore/CoreIntentName.swift), [`CoreIntentRules.swift`](../../Sources/TenonCore/CoreIntentRules.swift), [`WorkspaceIntentProvider.swift`](../../Sources/TenonApp/WorkspaceIntentProvider.swift) | high | finite read-only owner query for plugin/CLI/agent over the same catalog |
...
| `SP-FR-004` | The optional default maximum width **MUST** use the existing 1/3, 1/2, Full vocabulary, constrain every future creation path and every automatic horizontal growth after a pane leaves a tab — whether it closed or moved to another tab — never shrink a surviving pane, never exceed available space, keep a clicked cell inside a new pane, and leave both manual resize and a frame the operator pointed at unrestricted. | must | shipped | `@req-sp-fr-004` |
...
| `SP-FR-006` | A pane leaving a tab — by close or by cross-tab move — **MUST** deterministically absorb its geometry into a valid neighbor when possible, except that horizontal absorption stops at the SP-FR-004 maximum and leaves declined width empty; it **MUST** preserve unrelated panes and choose a valid active pane. Both departures **MUST** reflow the emptied tab through one implementation. A tab emptied by a **move** **MUST** close if another tab survives; a tab emptied by a **close**, or by abandoning a launcher reservation, **MUST** stay and show one `.empty` pane filling the grid, with a new pane identity, focused. | must | shipped | `@req-sp-fr-006` |
...
| `SP-FR-030` | A recorded session's résumé invitation (`AgentSessionResumeView`, offered through `AgentSessionResume.offer`) **MUST** reflect the current agent-detection result rather than whatever it was on the pane's first render. Agent detection is an async scan the host runs once at launch; a recorded-session pane opened before it resolves **MUST** update from stating the provider is not installed to offering Resume once detection actually finds it, with no user action required. | must | shipped | `@req-sp-fr-030` |
...
| SP-FR-030 | a recorded session's résumé invitation is not frozen at first render | `SpatialCanvasInteractionTests` content-key cache test |
...
| SP-FR-030 | shipped | `SpatialSlotCardView.configure`'s `agentSuffix` now covers `.agentSession` beside `.empty` | `SpatialCanvasInteractionTests.testAgentSessionContentRebuildsWhenAgentDetectionResolves` | not observed in the installed app; the scan's real ~2.1s window was not reproduced live, only its `[]`-then-populated shape |
...
| 2026-08-13 | *(superseded 2026-08-20 for close and reservation cleanup; still current for cross-tab move.)* A pane-level mutation that leaves a tab empty closes that tab whenever another tab survives. `workspace.pane.close.v2` and `dev.tenon.core-commands.pane.close.v2` replace their v1 contracts. | Empty source tabs were residue from closing or moving a final pane, not durable working state. The model applies the rule once across close, move, and reservation cleanup; the required last tab remains because every workspace must keep one tab. The extra tab removal changes observable side-effect meaning, so same-major evolution is forbidden and both superseded v1 paths are deleted. | SP-FR-006's rule that every final-pane close leaves an empty tab |
...
| 2026-08-17 | The maximum width follows the *kind of layout*, not the kind of gesture: automatic layout obeys it, a frame the operator pointed at does not. So the tab a pane left reflows under the maximum whether the pane closed or moved, `moveSlotToNewTab` and chip-drop auto-placement obey it, and the two drops that draw a highlight — `moveSlot(_:toTab:at:)` and `_:beside:edge:` — still adopt their promised frame exactly. `Workspace.removeSlot` is now the single implementation both departures reflow through, replacing `detachSlot`'s near-verbatim copy of `closeSlot`. | Operator report: closing a pane grew the neighbour to the configured maximum while dragging the same pane to a new tab grew it to the full canvas, and the pane landed full-width in the tab it arrived in — one fact, two widths. Root cause was structural rather than local: the policy travelled as `sizing: NewPaneSizing = .unlimited`, an optional parameter with a default, so obeying it was opt-in per call site and the whole cross-tab family had silently never opted in (`WorkspaceStore.swift:286-306` read `newPaneSizing` for close/add/split/duplicate/newTab and for no move). This narrows the 2026-08-14 row below rather than reversing it: its reason — the highlight must promise exactly the committed frame — only ever reached the two drops that draw one, and its "a move is not a creation" now reads "a move *into a frame the operator chose* is not a creation". Its consistency argument survives too, since the paths it compared (routed hole drop, chip auto-placement) are now told apart by whether a frame was promised, not by which code path ran. | the 2026-08-14 reading that no cross-tab move answers to the maximum |
...
| 2026-08-20 | A tab auto-closes only when its last pane **moves** to another tab. Closing the last pane, or abandoning a `+`/empty-canvas launcher reservation, leaves the tab standing with one fresh `.empty` pane over the full grid; `discardEmptySlot` on a tab's only pane is a no-op. `closeTabIfEmpty` now has exactly one caller family, the cross-tab move. | Operator report (T-193): the three call sites were treated as one event, so closing a pane made the tab silently vanish — a destination the operator never named. A move does name one, which is the whole difference. Distinguishing them also closes `b62d07e`'s pinned gap for free: `closeTab` refuses a workspace's last tab (`Workspace.swift:654-684`), so emptying it used to leave a 0-slot ghost that `TabContextPlacement.scopedPane` read exactly like a closed tab, failing every command in the tab's own right-click menu with "This space is no longer available". A tab that always holds a pane has nothing to resolve to nothing. The departing pane still leaves through `removeSlot` and still publishes `.slotClosed`, so surface-pool, plugin-resource and terminal-job teardown are unchanged; the placeholder arrives after it as a separate `.slotOpened`/`.slotFocused` pair with a new UUID, never the closed pane's identity reused. | **the 2026-08-13 row above, whose "the model applies the rule once across close, move, and reservation cleanup" is superseded** — the rule now applies once across move alone |
| 2026-08-20 | **Open, not decided by T-193:** whether `workspace.pane.close.v2` and `dev.tenon.core-commands.pane.close.v2` owe a `.v3` for no longer closing the tab they emptied. | The row above this one minted `.v2` precisely because *adding* that tab removal "changes observable side-effect meaning, so same-major evolution is forbidden". Removing it again is the same kind of change by the same rule, so the symmetric answer is a mint. T-193 did not make it: the mint reaches `CoreIntentName.swift`, `CoreCommandsPlugin.swift`, `plugins/core-commands/manifest.json`, `design-pane-slots.md`, `design-command-palette.md` and `architecture-interaction-boundaries.md` — none of them in T-193's claimed file set, on a tree several agents are editing. The behavior change itself is operator-directed and shipped; only the version question is outstanding, and it is stated here rather than left for a reader to notice. | — |
| 2026-08-18 | `SpatialSlotCardView.configure`'s cache-key `agentSuffix` now covers `.agentSession` beside `.empty`, so a recorded-session pane rebuilds when agent detection resolves. | Operator report (T-180, screenshot): a recorded-session pane's résumé invitation said Claude Code was not installed while it demonstrably was. Root cause: `agentSuggestions` reaches `AgentSessionResumeView` through `BuiltInSlotContentView`, but the 2026-08-08 row's own reasoning — content ignores state it does not consume, so the cache guard should not rebuild for it — was applied to `.agentSession` even though that content kind DOES consume `agentSuggestions`. Detection is an async ~2.1s scan (T-175); a pane opened before it resolves froze at `agentSuggestions == []` permanently, since neither the key's `agentSuffix` nor `.agentSession`'s `busValue` depended on it. `.agentSession` holds no PTY/WebView, so it earns the same treatment `.empty` already had. | narrows the 2026-08-08 row: "content ignores it" was true for every kind that does not read `agentSuggestions`, not for every kind besides `.empty` |

## 13. Verification receipts

- 2026-08-20, T-193, `SP-FR-006`: **red first** — the four rewritten tests plus one new one failed 10 assertions against the old model (`("Optional([])") is not equal to ("Optional([SlotContent.empty])")` for the workspace's last tab; `[slotClosed, tabClosed, tabSelected, slotFocused] != [slotClosed, slotOpened, slotFocused]` for the close events), green after. Seams: `WorkspaceTests.testCloseSlotUsesSmartAbsorptionAndShowsEmptyStateAfterItsLastSlot`, `.testCloseLastSlotInOnlyTabKeepsTheTabShowingAnEmptySlot`, `.testTheEmptyStateLeftByAClosedFinalPaneFillsTheGrid`, `.testDiscardOnlyEmptyReservationIsANoOpAndKeepsShowingEmptyState`, `TabContextPlacementTests.testATabEmptiedToTheWorkspacesLastPaneShowsItsEmptyStateAndStaysAddressable` (which pinned the `b62d07e` gap and now pins its fix), and `PaneProcessAndTabCloseIntentTests.testPaneCloseLeavesItsEmptiedTabStandingWithAnEmptyPane` at the intent layer. The three `moveSlot`/`moveSlotToNewTab` auto-close tests passed unedited throughout, which is what proves the move half of the rule was left alone. Full suite **2400 / 0**.
...
- 2026-08-17, T-174, `SP-FR-004` / `SP-FR-006`: a pane that moves obeys the width rule a
  pane that closes obeys. Four new tests written before the fix, three red on the first run
  and the fourth green from the start — which is the point of writing it: the drop that draws
  a highlight was already correct and had to *stay* correct through the change. The reds were
  `testDraggingAPaneOutLeavesTheNeighborExactlyWhereClosingItWould` at
  `width: 12` against an expected `4`, `testAPaneDroppedOnTheTabBarOpensAtTheConfiguredMaximum`,
  and `testAPaneDroppedOnATabChipTakesFreeCanvasBoundedByTheMaximum`; the standing green was
  `testAPaneDroppedOnAHighlightedRegionStillAdoptsThatRegionExactly`. All four green after,
  `NewPaneSizingTests` 27 / 0, full suite **2284 / 0**. The refactor is covered by the tests
  that already existed: `closeSlot` lost 29 lines to `removeSlot` while
  `WorkspaceTests`'s exact-event assertions (`testMoveSlotToNewTabReparentsKeepingIdentityAndReflowsSource`
  and the close suites) held the event sequences unchanged. NOT VERIFIED: no drag has been
  performed on an installed build; the suite proves the widths, not the gesture that asks for
  them.
...
- 2026-08-18, T-180, `SP-FR-030`: red first —
  `testAgentSessionContentRebuildsWhenAgentDetectionResolves` asserted `card.contentKey`
  after `agentSuggestions` changed from `[]` to a Claude install and failed on identical
  keys (`agent-session:claude:0199f0c1-...` unchanged), matching the bug: an `.agentSession`
  pane's résumé invitation frozen at whatever `agentSuggestions` was on first `configure`.
  Green after folding `.agentSession` into `SpatialSlotCardView`'s `readsAgentSuggestions`
  gate beside `.empty`. Scope `SpatialCanvasInteractionTests` **75 / 0**. Full suite
  **2361 / 0** (a same-session prior run reported 2361/1 with no failing-test name visible
  in the captured tail; an immediate full re-run reproduced 0 failures, consistent with this
  repo's documented pattern of a concurrent peer session's own edit landing mid-run rather
  than a defect in this change — see the T-161 receipt above for the same shape). NOT
  VERIFIED: the installed app, and the real ~2.1s detection-scan window; the test drives the
  same `[]`-then-populated `configure` shape the app produces, not the scan's timing itself.
...
| 2026-08-18 | Added SP-FR-030 for T-180: a recorded session's résumé invitation must reflect agent detection as it resolves, not freeze at the pane's first render. | Claude |
