...
| `AL-M-004` invented/shared Timeline anchors rendered | zero, and unexpressible: a milestone names a range over the numbered digest rather than fact IDs | validator mutation tests |
...
each covering between three facts and half the digest, titles 80 chars and prose fields 400 chars; synthesizer process 45 seconds of
...
| `AL-FR-027` | A milestone **MUST** name the stretch of evidence it covers as an inclusive range over the numbered digest, never as transcribed fact IDs; a range outside the digest **MUST** be held inside it. Anchors, their labels, and time spans **MUST** be host-derived from the facts that range contains. | shipped | `@req-al-fr-027` |
| `AL-FR-028` | Milestone grouping **MUST** be a partition with at least three facts each: ranges **MUST** be read oldest first and **MUST NOT** overlap, and facts between ranges are the ones the reading left out. Settled **MUST** be refused over observed open work anywhere in the range; no session verdict may be synthesized. | shipped | `@req-al-fr-028` |
...
| `AL-FR-049` | The silence deadline **MUST** apply only while the CLI publishes a heartbeat. The stretch between the request leaving for the API and the first frame of the reply carries no frame at all, so it **MUST** be treated as accounted for and left to the ceiling — the same answer a provider that streams nothing already gets — and the pane **MUST** say the run is waiting for the model rather than reporting the line it shows while reading. The stretch from process start to the CLI's own first frame carries no frame either — it holds the CLI's cold start and the handover of a prompt larger than the operating system's pipe buffer — so it **MUST** be accounted for on the same rule. The first frame of the reply **MUST** revoke that account, putting the measured bound back in charge. | shipped | `@req-al-fr-049` |
...
| `AL-FR-054` | A reading **MUST** record its start and its terminal state — provider, prompt size, deadlines in force, elapsed time, exit status and the reason it ended — to the host's own log, so a reported failure names which deadline stopped it without the pipeline being re-run outside the app. | shipped | `@req-al-fr-054` |
| `AL-FR-055` | Which readers exist **MUST** be answered from executable directories alone, **MUST** be asked by the pane rather than by the account that displays it, and a found answer **MUST** be kept for that pane's life; no shell history may be read to decide it, re-opening Timeline **MUST NOT** ask again, and only an answer that found nothing may be re-asked. | shipped | `@req-al-fr-055` |
...
[`agent-lens.feature`](agent-lens.feature) maps all 66 requirements. Reducer/decoder/tailer,
...
| 055 | `AgentCLITimelineSynthesizer.installedProviders(locator:)`/`installed(provider:locator:)`, `AgentLensViewModel.detectReaders`/`askWhichReadersExist()`/`loadAvailableReaders()` | shipped; `AgentReadingOptionsTests`, three mutations |
...
| 2026-08-17 | Which readers exist is asked by the pane when it opens, from executable directories only, and the found answer is kept. Measured on the shipped types: `AgentLaunchDetector.scan()` **2.107 s**, `AgentExecutableLocator.scan()` **1.20 ms** — 1751×, because the detector reads a 512 KB tail of every shell history file and tokenizes each line to guess habitual arguments, which neither `installedProviders` nor `installed(provider:)` keeps. Laziness was the reason the view owned the question; at 1.20 ms there is nothing left to defer, and deferring it is what put the reader picker on screen after the row it belongs to. | the view asking on appearance, through the detector, once per visit |
...
| 2026-08-16 | `main`, session `ee649e9f` | `AL-FR-027`, `AL-FR-028`, `AL-FR-049`, `AL-FR-054` (T-169) | Reported from a live pane: "The reading stopped responding after 45s of silence". Diagnosed by re-running the pipeline outside the app on CLI 2.1.233, with the arguments `arguments(provider:model:)` builds and three real transcripts. **The heartbeat T-137 measured is still there** — 69 `system/thinking_tokens` frames at ~2.1 s over a healthy 201.6 s run, worst *unexplained* gap **2.2 s** against a 45 s budget — so the deadline is not misfiring mid-reply. It misfires **before** it: `AgentRunActivity` armed the budget from `process.run()`, and the first frame lands **15.7 s** into an idle run and **25.3 s** into one of eight concurrent readings, 56% of the budget spent before the CLI can speak. Two things fill that window and both grow with load — node cold start, and a `write` of a 48.8–82.6 KB prompt through a pipe buffer measured at **65 536 bytes**, blocking 2.2–11.0 s. Environment measured and excluded (first frame 7.9 s under a shell environment, 9.8 s under the launchd-minimal one a bundled app carries). Startup is now accounted for on the rule this file already applied to the request in flight, with the ceiling as its only bound. **Red first**: `testStartupQuietIsNotEvidenceBecauseTheCliCannotSpeakDuringIt` failed alone (1 of 17) before the initialiser changed. Found on the way and fixed: a reading of a 215-fact session returned eleven usable milestones and the anchor `message-c9f48eda-83a9-4b4d-b8f9-none`, a true UUID prefix with an invented tail, and **277 s of work were discarded over one of ~90 hand-copied ids**. Milestones now name an inclusive range over numbered facts; `inventedAnchor`, `unanchored` and `echoesOneFact` were **deleted rather than relaxed**, because a span cannot cite a fact the session lacks and a span of three cannot be one row wearing a heading. Measured on the same three sessions, v1 → v2: **276.9/247.3/379.3 s → 45.6/77.0/55.4 s**, replies 8.3–11.3 KB → 5.5–6.6 KB, prompts −13…14%, and **1 of 3 refused → 3 of 3 accepted**. `AgentSessionTimelineTests` **33 / 0**, `AgentReadingSilenceTests` **17 / 0**, `AgentCLIRetryTests` **9 / 0**, `AgentReadingOptionsTests` **13 / 0**. | **That the startup window is what killed the operator's run is NOT established** — no crossing of 45 s was reproduced here, and their real load (Swift compiles beside several agent panes) is heavier than eight idle node starts. `AL-FR-054` exists because of that gap: the feature logged nothing at all, so the report could only be reconstructed by re-running the pipeline. The suite did not catch the bound this change broke — 2291 / 0 green while all three live readings were refused for `oversizedSpan`, because the old cap of 24 was sized for citations and a real phase measures 28–43 facts; the fixtures had no span that wide. The replacement, half the digest, is a product judgement about a reading being more than one milestone, not a measurement. No pane was photographed. |
...
| 2026-08-17 | `main`, session `d653bc85` | `AL-FR-055` (T-175) | Reported from a live pane: opening Timeline shows "default gì đó", then a moment later shows Claude Code or Codex. Diagnosed by reading the composition, then measured on the shipped types before the first edit. The first frame draws with `availableReaders == []`, so `readerPicker`'s `count > 1` hides it and the leftmost control is `modelPicker` reading **"Default model"**; when the scan lands the reader picker is *prepended* and pushes the row sideways. The wait is not the question's cost but the detector's: **`AgentLaunchDetector.scan()` 2.107 s against `AgentExecutableLocator.scan()` 1.20 ms — 1751×** (mean of five, after a warm run; this machine's `.bash_history` is 4.8 MB and `fish_history` 895 KB, and the detector takes a 512 KB tail of each, then tokenizes all 23,265 lines to guess habitual arguments). `installedProviders` kept only the agent of each suggestion and `installed(provider:)` kept only `match.executableURL`, so **both discarded every byte of it** — and `installed(provider:)` is on the run path, so each "Read this session" paid 2.1 s before the CLI was launched. Nothing cached either: the account switch destroys and rebuilds the view, so `.task` re-ran the whole scan on every visit to Timeline, in every pane. Three changes: both functions ask the locator; `loadAvailableReaders()` keeps a found answer and re-asks only an empty one; and `start()` asks, so the answer is on hand for the invitation's first frame. The tests were written first but had to land with the signature they name — `Tests/` is one target every concurrent agent shares — so the red is **three mutations, one at a time, the file restored byte-identical between each**: the cache guard removed → `testTheReaderScanIsAskedOncePerPane` at `("3") is not equal to ("1")`; `askWhichReadersExist()` removed from `start()` → `testThePaneAsksWhichReadersExistBeforeTimelineIsEverOpened` at `("[]")` against both CLIs; `installedProviders` returning `allCases` → `testAReaderWithNoBinaryIsNotOffered`. `AgentReadingOptionsTests` **13 → 18 / 0**, `AgentSessionTimelineTests` **33 / 0**, full suite **2289 / 0** in 138 s. Found on the way: `SettingsView` and `CompanionStructuredOutputRunner` already asked the locator, so this makes Agent Lens consistent with them rather than inventing a route. | **The layout shift is removed by a head start, not by a reserved slot** — a pane opens minutes before anyone clicks Timeline, but a pane opened and clicked within the same millisecond would still draw the row once without the picker; `readerPicker` keeps its `count > 1` rule, and a machine with one CLI never sees it at all. **No pane was photographed**: `TENON_VIEW_SNAPSHOT` reaches plugin views and the Timeline snapshot writer mounts the reading, not the invitation mid-scan, so the jump itself has no visual receipt before or after. The 2.107 s figure is one machine's history size; a person with small shell histories would have seen a fraction of it, and nothing here bounds what the detector costs in general. `ContentView.swift:161` and `AgentIntentProvider.swift:53` still call `AgentLaunchDetector` and still pay it — correctly, since they use the arguments it guesses — but that the launcher surface spends 2.1 s on it is untested and belongs to another PRD. |
...
| 2026-08-17 | Added FR-055; corrected the mapped-requirement count from 58 to 66 | The reader scan was measured at 2.107 s against 1.20 ms for the question it answers, and no requirement said who asks it or how often. The count had been stale since FR-051 was added; every one of the 66 is tagged in the feature file, verified by comparing both lists. |
