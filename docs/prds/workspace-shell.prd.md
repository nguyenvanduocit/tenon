...
| `WS-FR-037` | A workspace row's context menu **MUST** offer Sleep, which releases every live terminal and plugin-webview resource the workspace owns while leaving its `Workspace` value — tabs, panes, layout, UUIDs — completely unchanged; if the workspace is currently active, Sleep **MUST** select another workspace first. Sleep **MUST** be reachable both DIRECT from the row and through `workspace.sleep.v1` (`{plugin, cli, agent}`), both calling the same typed action. Reopening any pane of a slept workspace **MUST** materialize a fresh resource with no restored process or scrollback — wake is not a distinct code path, it is the same lazy materialization restore already uses. | must | shipped | `@req-ws-fr-037` |
| `WS-FR-038` | A workspace row's context menu **MUST** offer Move to Background, which sets `Workspace.visibility` to `.background` without touching any live resource, reachable both DIRECT from the row and through `workspace.visibility.set.v1`. Setting `.background` **MUST** be refused when no other `.visible` workspace remains; backgrounding the active workspace **MUST** reselect a neighbor exactly like removing a workspace does. The sidebar's main list **MUST** exclude `.background` workspaces; a "Backgrounded" section **MUST** list them by name and restore a chosen one to `.visible` and active in one action. `visibility` **MUST** persist across relaunch. | must | shipped | `@req-ws-fr-038` |
...
| WS-FR-037…038 | shipped | `WorkspaceVisibility`, `Workspace.visibility`, `WorkspaceCatalog.setVisibility`, `WorkspaceSleepAction`, `WorkspaceIntentProvider` (`workspace.sleep.v1`, `workspace.visibility.set.v1`), `PluginWebSurfacePool.disposeSurfaces`, `AppComposition`'s late-bound Sleep teardown, `WorkspaceSidebarView`'s Sleep/Move to Background row actions and `BackgroundedWorkspacesSection` | `WorkspaceVisibilityTests` (8), `WorkspaceSleepActionTests` (3), `WorkspaceIntentProviderTests` (+5), `PluginWebSurfacePoolTests` (+1), `WorkspaceCatalogPersistenceTests` (+2), `WorkspaceSidebarVisibilityTests` (4), catalog/fitness inventory tests, `TENON_SIDEBAR_SNAPSHOT` with a backgrounded workspace present | no live-app pointer/drag journey against a slept or backgrounded row; the sidebar's sleep-indicator badge described in the design spec was descoped by explicit product decision |
...
| 2026-08-20 | Sleep makes zero `Workspace` domain-model change; it is a host-only action that narrows the set already passed to `SurfacePool.retainOnly`/`PluginWebSurfacePool`. | wake needed to be indistinguishable from opening a workspace after relaunch — both product decisions, not implementation shortcuts — and the existing lazy-materialization path already gives exactly that state for free once a slot's live surface is released; a new `SlotContent`/domain flag would have made "asleep" a second thing to keep in sync with "no surface exists" instead of the same fact | a new `SlotContent` case mirroring `.agentSession`, or a per-slot "asleep" flag on `Workspace` |
| 2026-08-20 | Wake never resumes a live agent session with `--resume`; every freshly materialized pane is a plain new shell. | explicit product decision: wake was chosen to behave identically to quit-and-reopen, not to attempt session continuity. `AgentSessionResume.swift`'s composer remains available for a future, separately scoped enhancement | auto-resuming a recognized agent's session on wake via `AgentLaunchComposer` |
| 2026-08-20 | `WorkspaceSleepAction` is a late-bound mutable closure box, constructed before `AppIntentRuntime` and assigned its real body only after `PluginHost` exists. | `PluginHost.init` takes `intentRuntime.kernel`, so `PluginHost` cannot exist before `AppIntentRuntime`, yet `workspace.sleep.v1`'s handler needs `PluginHost` (to resolve which `WebSurfaceKey`s a workspace's plugin panes own) at construction time. A settable `perform` closure — the same shape `PluginHost.onPluginLifecycleChanged` already uses — lets both the sidebar's Sleep button and the intent call one typed implementation (invariant 6) without reordering composition | passing `PluginHost` into `AppIntentRuntime.init` (impossible, circular) or duplicating the teardown logic in two places |
| 2026-08-20 | The sidebar's sleep-indicator badge is not built in this pass. | workspace-level "is this asleep" state does not live anywhere today — `SurfacePool` is keyed by slot, not workspace — and adding a tracker correctly means wiring it into `SurfacePool.surface(for:)`'s materialization path for auto-clear. The operator confirmed (AskUserQuestion) skipping it; no acceptance criterion in the design spec depends on it | a `Set<UUID>` tracker on `SurfacePool` or a new observable object |
| 2026-08-20 | Drag-to-reorder operates entirely in visible-workspace-list space; only the final `store.moveWorkspace` call translates to the full catalog's absolute index via `absoluteWorkspaceIndex(forVisibleDestination:in:)`. | rendering `WorkspaceRowList` over `visibleWorkspaces` instead of `store.catalog.workspaces` keeps every existing `WorkspaceReorder` geometry computation (`rowStrip`, `horizontalBand`, insertion index) unchanged and self-consistent; a backgrounded workspace never renders a row to drag, so reorder interaction with one is out of scope by construction rather than by a special case | filtering only the `ForEach` and leaving reorder math over the unfiltered array (breaks the `rows.count == workspaces.count` parity guard the moment any workspace is backgrounded) |
...
- 2026-08-20, `WS-FR-037`/`WS-FR-038` (T-194): brainstormed against Orca's "Sleep Worktree"
  reference implementation (`docs/superpowers/specs/2026-08-20-sleep-workspace-design.md`,
  `docs/superpowers/plans/2026-08-20-sleep-workspace.md`). `WorkspaceVisibilityTests` (8)
  pins `setVisibility`'s round-trip, the last-visible-workspace refusal, and the
  active-workspace-handoff shape against `removeWorkspace`. `WorkspaceSleepActionTests` (3)
  proves Sleep releases exactly the target workspace's own terminal surfaces, leaves every
  other workspace's untouched, and hands off the active workspace first. `PluginWebSurfacePoolTests`
  gained `testDisposeSurfacesForPluginViewSlotsReleasesOnlyTheNamedSlotsWebviews` (2 separate
  plugin installations, proving disposal is scoped to the named slots only).
  `WorkspaceIntentProviderTests` (+5) covers both new intents' success, not-found, and
  refusal paths. `WorkspaceCatalogPersistenceTests` (+2) covers visibility's round-trip and
  its pre-existing-document forward compatibility. `WorkspaceSidebarVisibilityTests` (4)
  proves `absoluteWorkspaceIndex` lands a visible-list destination at the correct absolute
  catalog index around an interleaved backgrounded workspace. Two build failures the plan did
  not foresee: `WorkspaceEvent`'s two other exhaustive switches (`RecentStore.workspaceID`,
  `PluginHost.emit`'s bus-event mapping) needed the new case, and the intent catalog's exact
  counts were pinned in two more places than anticipated
  (`testConcurrentInstallCompilesIntoAuthoritativeKernelExactlyOnce`'s revision/definition/
  contract/dispatchRule counts, and `testAudienceExposureProviderAndResourcePoliciesAreCoherent`'s
  explicit per-lane `Set` literal). `TENON_SIDEBAR_SNAPSHOT` at 232×460 with one workspace
  backgrounded confirms the main list excludes it and the "Backgrounded" section names it.
  Full suite **2430 / 0**.
...
| 2026-08-20 | Added `WS-FR-037`: Sleep releases a workspace's live terminal/plugin-webview resources with zero domain-model change, reachable DIRECT from the sidebar and through `workspace.sleep.v1`. Added `WS-FR-038`: Move to Background hides a workspace from the sidebar's main list via a new `Workspace.visibility` field, without touching any live resource, reachable DIRECT and through `workspace.visibility.set.v1`. | Claude (T-194) |
