...
| `CMD-FR-025` | `EmptyPaneOfferings.views` (the empty tab/pane card's "Open a view" section) **MUST** offer every plugin-manifest command that declares `launcher: true, fillsPane: true`, other than the terminal command already drawn as the card's own CTA, so `LauncherMenu`'s "Open a view" tile grid and the empty-pane card's grouped layout never disagree about which views exist. The id→`SlotContent` mapping **MAY** stay hand-written until `SlotContent` stops being a closed enum (T-149); a headless test **MUST** fail the day a manifest's `fillsPane` set changes without a matching edit here. | must | shipped | `@req-cmd-fr-025` |
...
| CMD-FR-025 | shipped | [`EmptyPaneOfferings.swift`](../../Sources/TenonApp/EmptyPaneOfferings.swift) (`views` gains the two `fillsPane` entries it was missing — Kanban's board, Agent Sessions) | `EmptyPaneSearchTests.testOpenAViewOffersEveryFillsPaneCommandInTheRealPluginInventory` — walks the real `plugins/` inventory via `PluginLoader`, red first (`dev.tenon.kanban.open.v1`/`dev.tenon.claude-sessions.open.v1` both `fillsPane` with no matching `views` entry), green after | the id→`SlotContent` map inside the new test is itself hand-written, same as `views` — a plugin adding a `fillsPane` command still needs both edited together; the derivable version needs T-149 |
...
| 2026-08-19 | **`EmptyPaneOfferings.views` gains the two `fillsPane` commands it was missing — Kanban and Agent Sessions — and a headless test now walks the real `plugins/` inventory so this specific drift fails loudly the next time a manifest changes (`CMD-FR-025`, T-191).** | Operator-reported from a screenshot comparing `LauncherMenu`'s "Open a view" tile grid (6 items, read live off `CommandIndex`) against the empty-pane card's own "Open a view" grid (4 items) side by side — visibly two different lists behind what `§3`'s vocabulary table calls one shared launcher. T-188 had already named the root cause (`views` is the "second command registry" `§3` marks as a non-goal) and deliberately left it, because the derivable fix needs `SlotContent` to stop being a closed enum (T-149, effort L, touches `Workspace.swift`/`WorkspaceCatalogStore.swift` and 5 exhaustive switches) — out of proportion for a presentation-parity fix. Chose the scope T-149 itself implicitly asked for in the meantime: keep the two lists in hand-written sync, but make the sync failure loud instead of silent, by asserting the real manifest inventory's `fillsPane` set against `views` in a test rather than trusting a human to notice the count drift again. | doing T-149 now to derive `views` instead of hand-syncing it — correct root cause, but effort L against files other sessions touch, and far past what a "why do these two popovers look different" report asked for |
...
| 2026-08-19 | current sources, `swift test --filter EmptyPaneSearchTests/testOpenAViewOffersEveryFillsPaneCommandInTheRealPluginInventory` | `CMD-FR-025` | red first — `dev.tenon.kanban.open.v1`/`dev.tenon.claude-sessions.open.v1` both reported `fillsPane` with no matching `EmptyPaneOfferings.views` row — against the pre-fix `views`; green after appending both | none known |
| 2026-08-19 | current sources, `swift test` | full suite | passed **2397 / 0** in 172 s | no live `NSPopover` pixel check, same gap this section already carries for `CMD-FR-011`/`CMD-FR-020`/`CMD-FR-022`/`CMD-FR-024` |
...
| 2026-08-19 | `EmptyPaneOfferings.views` ("Open a view" in the empty tab/pane card) gains Kanban and Agent Sessions, and a new test pins it against every `fillsPane` command the real `plugins/` inventory declares (`CMD-FR-025`, T-191). | Operator-reported, in Vietnamese: two launcher popovers, opened back to back, offered visibly different view counts. Root cause was T-188's already-named, deliberately-deferred gap — `views` is a hand-maintained list, `LauncherMenu`'s own equivalent section reads `CommandIndex` live — and the real plugin inventory had grown two `fillsPane` commands (kanban, claude-sessions) past what `views` still listed. The full fix (T-149) rewrites `SlotContent` into a registry; this closes the immediate parity gap and adds `EmptyPaneSearchTests.testOpenAViewOffersEveryFillsPaneCommandInTheRealPluginInventory` so the next drift is a red test, not a screenshot. |
