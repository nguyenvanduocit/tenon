...
| Lifecycle | `shipped`; bounded health diagnostics and the read-only process Resource Monitor both ship. T-114 closed the signed-app feasibility question — the Hardened Runtime does not restrict `libproc` — leaving the signed Release benchmark and XCUITest receipts outstanding |
...
| `DRM-NFR-012` | evidence | Core, hosted, real-Ghostty, UI/accessibility, performance, signed Release, and live receipts **MUST** prove their respective claims at one reviewed source SHA. | partial; core/hosted/real-`libproc` receipts exist and T-114 proved the Hardened Runtime leaves `libproc` unrestricted, so what remains outstanding is the signed Release *benchmark* and XCUITest | `@req-drm-nfr-012` |
...
| 2026-08-10 | current dirty tree (T-114, session `407fc72f`) | one probe binary making this sampler's exact `libproc` calls, signed twice and differing only in the Hardened Runtime flag, on Darwin 25.4/arm64 | **The signed-app feasibility question this PRD has carried since 2026-07-30 is answered: the Hardened Runtime does not restrict `libproc`.** The design record's outstanding item read that the app "is ad-hoc signed with no sandbox and no hardened runtime … so no entitlement can restrict `libproc` here", which was reasoning rather than a receipt. Measured, the two runs are identical: `proc_listpids` 638 pids, `PROC_PIDTBSDINFO` readable 419, `EPERM` 219, `PROC_PIDTASKINFO` readable 419, `proc_pidpath` 635. The 219 refusals are processes owned by root or other users and are present unhardened too. A second finding bounds how the app can ever be signed: with an **ad-hoc** signature the Hardened Runtime refuses to load Tenon's own embedded frameworks (`different Team IDs`), so hardening belongs only to a build signed by one real identity — see PRD-015 `ENQ-FR-035…037` and [`releasing.md`](../releasing.md) | the p95 sampling benchmark still has no signed Release receipt, and no live multi-workspace comparison; this closes the *feasibility* question, not the *performance* one |
