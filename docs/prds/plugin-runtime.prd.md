...
| `PRT-FR-042` | Streaming process launch **MUST** own a POSIX process group (or equivalently race-free descendant set) so cancel/overflow/retirement terminates the leader and descendants before process-tree containment is claimed. | shipped | `@req-prt-fr-042` |
...
| 2026-08-14 | T-167, shared `main` | All ten shipped bundled plugins now use `runtime: bundled-swift`; their `main.js` entrypoints are deleted. The compiled ports preserve manifest-backed boundaries, including view callbacks, instance state, storage, timers, watches, repository parsing, Claude/Codex scanning, and Kanban writes. Focused Xcode coverage is 54/0, `WorkspaceScopedViewStateTests` is 9/0, `DomainTagFitnessTests` is 7/0, and the full SwiftPM suite is 2,285/0; `xcodegen generate` is deterministic. | Compiled bundled plugins now move on the app's release cadence; third-party JavaScript remains supported and hot-reloadable. |
...
| 2026-08-12 | T-140, shared `main` | `PRT-FR-042` shipped: `process.stream` now launches through `PluginStreamProcess` (`posix_spawn` + `POSIX_SPAWN_SETPGROUP`), and cancel/overflow/retirement `killpg` the group with a 120 ms SIGTERM→SIGKILL escalation. `swift test --filter PluginStreamProcessTests` 4/0; full suite **2074/0**. Mutation-checked twice: removing `killpg` leaves the forked descendant alive and reddens the test. The first mutation round *survived* — the test had been matching `sleep 4084` by name and finding `TerminalJobTerminationTests`' own fixture on the same machine, so it was measuring that test's cleanup. Rewritten to identify the child by parentage; a second parsing bug (`.whitespaces` does not trim `ps`'s trailing newline) had reported a live process as "in group none" | a command that calls `setsid` or daemonizes into `launchd` leaves the group and survives; stated in `docs/plugin-author-guide.md` rather than implied away, per `PRT-G-005` |
