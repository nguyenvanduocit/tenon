---
title: workspace.pane.focus-next.v1
...
Cycles focus within the active tab in invocation scope.
...
```js
const result = await tenon.intents.send("workspace.pane.focus-next.v1", {})
...
```sh
tenon-cli intent send workspace.pane.focus-next.v1 --input '{}'
```

Ask your own build for this same contract with `tenon-cli intent describe workspace.pane.focus-next.v1`.
