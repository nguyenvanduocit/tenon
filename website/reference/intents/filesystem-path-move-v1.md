...
Moves a file or directory without replacing an existing destination.
...
|---|---|---|---|
| `destinationPath` | yes | `string` | length 1–16384 |
| `sourcePath` | yes | `string` | length 1–16384 |
...
```js
const result = await tenon.intents.send("filesystem.path.move.v1", {})
...
```sh
tenon-cli intent send filesystem.path.move.v1 --input '{}'
```

Ask your own build for this same contract with `tenon-cli intent describe filesystem.path.move.v1`.
