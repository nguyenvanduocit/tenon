...
Opens a path with the trusted default or an explicitly approved provider.
...
```js
const result = await tenon.intents.send("file.open.v1", {})
...
```sh
tenon-cli intent send file.open.v1 --input '{}'
```

Ask your own build for this same contract with `tenon-cli intent describe file.open.v1`.
