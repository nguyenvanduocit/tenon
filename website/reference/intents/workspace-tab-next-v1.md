...
Selects the next tab in the workspace identified by invocation scope.
...
```js
const result = await tenon.intents.send("workspace.tab.next.v1", {})
...
```sh
tenon-cli intent send workspace.tab.next.v1 --input '{}'
```

Ask your own build for this same contract with `tenon-cli intent describe workspace.tab.next.v1`.
