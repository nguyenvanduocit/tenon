---
title: workspace.pane.content.set.v1
...
Replaces content in the pane identified by invocation scope.
...
```js
const result = await tenon.intents.send("workspace.pane.content.set.v1", {})
...
```sh
tenon-cli intent send workspace.pane.content.set.v1 --input '{}'
```

Ask your own build for this same contract with `tenon-cli intent describe workspace.pane.content.set.v1`.
