...
## Where the CLI capability evidence lives

Read [`docs/reports/2026-08-11-cli-capability-survey.html`](docs/reports/2026-08-11-cli-capability-survey.html)
before proposing a `tenon-cli` capability, and before rejecting one. It is a self-contained
page: open it in a browser, filter the 379-row matrix by domain, status, or effort, and click
a row to see the evidence and the adversarial refute verdict behind that cell.

It measures `tenon-cli` (34 CLI-callable intents) against orca (223 commands) and herdr
(80 socket methods, 24 push events) across 18 capability domains. The single number to
distrust is the 223 "absent" total, because it merges three unlike things:

- **Refused on purpose.** Orchestration, browser/computer-use, remote pairing. `VISION.md:8-9`
  gives the operator one place to intervene *"without taking ownership of agent planning,
  spawning, scheduling, or execution"*, and `docs/prds/cli-control.prd.md:126` refuses remote
  again. Closing these would make Tenon a worse orca, not a better Tenon.
- **One catalog entry away.** The app already computes it and only the UI can see it —
  `SurfacePool.terminalProvenance`, `Workspace.closeTab`, `Workspace.resizeSlot`, the paneID
  that `workspace.pane.split.v1` computes and discards. Adding a `cli` audience over the
  existing typed service breaks no invariant.
- **Blocked by one architectural fact.** PTY ownership sits in the AppKit process
  (`Sources/TenonApp/TenonApp.swift:489-510`), so headless, detach/reattach, live handoff and
  remote attach are four names for the same missing boundary. These need a product decision
  before they need code.

The report is dated evidence and is never revised to stay current. Current source and current
tests win any disagreement with it. Work it justifies goes into `.kanban` and the owning PRD's
decision log, which is where status is allowed to live.
