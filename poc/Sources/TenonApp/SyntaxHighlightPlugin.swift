...
final class SyntaxHighlightCoordinator {
    ...
    init(
        ...
        self.textContentManager = textView.textContentManager
        ...
        // Empty fonts table (see SyntaxHighlighting.theme) → this keeps the editor's
        // own font instead of resetting to the theme's.
    ...
    private func injectedHighlightsQuery(for language: SyntaxLanguage) -> SwiftTreeSitter.Query? {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let query = try? SwiftTreeSitter.Query(
                language: Language(language: language.parser),
                data: data
            )
            DispatchQueue.main.async { [weak self] in
